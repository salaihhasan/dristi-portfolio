...
export const ProceduralMicroscope: React.FC<ProceduralMicroscopeProps> = ({
  ...
  const brassDetail = new THREE.MeshStandardMaterial({
    color: '#B65F6D', // Subtle dusty rose accent on lens rings
    metalness: 0.65,
    roughness: 0.28,
  });
  ...
  useFrame((state, delta) => {
    ...
    groupRef.current.position.y = Math.sin(state.clock.elapsedTime * 0.8) * 0.05 - 0.2;
    ...
    if (interactive) {
      ...
      groupRef.current.rotation.z = THREE.MathUtils.lerp(groupRef.current.rotation.z, -targetTiltY * 0.5, delta * 2);
    }
  });
  ...
  return (
    <group ref={groupRef} position={[0, -0.2, 0]} scale={[1.3, 1.3, 1.3]}>
      ...
      <mesh material={ivoryEnamel} position={[0, -1.1, 0]} castShadow receiveShadow>
        <cylinderGeometry args={[0.85, 0.95, 0.22, 32]} />
      </mesh>
      <mesh material={darkCharcoalTrim} position={[0, -1.22, 0]}>
        <cylinderGeometry args={[0.96, 0.98, 0.06, 32]} />
      </mesh>

      {/* 2. Lower Pillar & Illuminator */}
      <mesh material={chromeAccent} position={[0, -0.85, 0]}>
        <cylinderGeometry args={[0.22, 0.28, 0.35, 24]} />
      </mesh>
      {/* Light Field Diaphragm (Substage light source) */}
      <mesh material={darkCharcoalTrim} position={[0, -0.65, 0.2]}>
        <cylinderGeometry args={[0.26, 0.26, 0.12, 24]} />
      </mesh>
      <mesh material={brassDetail} position={[0, -0.58, 0.2]}>
      ...
      <group position={[0, 0, -0.25]}>
        {/* Lower Spine */}
        <mesh material={ivoryEnamel} position={[0, -0.3, 0]} rotation={[0.2, 0, 0]}>
          <boxGeometry args={[0.32, 0.85, 0.35]} />
        </mesh>
        {/* Upper Curved Arch */}
        <mesh material={ivoryEnamel} position={[0, 0.35, 0.12]} rotation={[-0.35, 0, 0]}>
      ...
      <group position={[0, -0.25, 0.18]}>
        {/* Main Stage Plate */}
        <mesh material={darkCharcoalTrim} castShadow receiveShadow>
          <boxGeometry args={[1.15, 0.08, 1.05]} />
        </mesh>
        {/* Slide Glass Specimen with Blood Smear Tint */}
        <mesh material={glassStage} position={[0, 0.06, 0]}>
          <boxGeometry args={[0.75, 0.02, 0.32]} />
        </mesh>
        <mesh material={brassDetail} position={[0, 0.08, 0]}>
          <cylinderGeometry args={[0.08, 0.08, 0.015, 16]} />
        </mesh>
        {/* Stage Clips */}
        <mesh material={chromeAccent} position={[-0.42, 0.07, 0]} rotation={[0, 0, 0.1]}>
      ...
      <group ref={nosepieceRef} position={[0, 0.45, 0.22]}>
        {/* Turret Disc */}
        <mesh material={darkCharcoalTrim} rotation={[0.1, 0, 0]}>
          <cylinderGeometry args={[0.36, 0.42, 0.14, 24]} />
        </mesh>

        {/* Objective 1: High Power (100x Oil Immersion - Rose Ring) */}
        <group position={[0, -0.28, 0.08]} rotation={[-0.05, 0, 0]}>
          <mesh material={chromeAccent}>
            <cylinderGeometry args={[0.09, 0.12, 0.38, 16]} />
          </mesh>
          <mesh material={brassDetail} position={[0, -0.12, 0]}>
            <cylinderGeometry args={[0.095, 0.095, 0.06, 16]} />
          </mesh>
        </group>

        {/* Objective 2: Medium Power (40x) */}
        <group position={[-0.18, -0.22, -0.12]} rotation={[0.2, 0, -0.3]}>
          <mesh material={chromeAccent}>
        ...
        <group position={[0.18, -0.2, -0.12]} rotation={[0.2, 0, 0.3]}>
          <mesh material={chromeAccent}>
      ...
      <group position={[0, 0.82, 0.1]}>
        ...
        <mesh material={ivoryEnamel} rotation={[-0.25, 0, 0]}>
          <boxGeometry args={[0.34, 0.42, 0.48]} />
        </mesh>

        {/* Eyepiece Tube (Ocular) */}
        <group position={[0, 0.28, 0.14]} rotation={[-0.45, 0, 0]}>
          <mesh material={chromeAccent}>
            <cylinderGeometry args={[0.11, 0.12, 0.45, 20]} />
          </mesh>
          {/* Rubber Eyecup */}
          <mesh material={darkCharcoalTrim} position={[0, 0.24, 0]}>
      ...
      <group position={[0, -0.22, -0.22]}>
        {/* Transverse Axle */}
        <mesh material={chromeAccent} rotation={[0, 0, Math.PI / 2]}>
          <cylinderGeometry args={[0.06, 0.06, 0.85, 16]} />
        </mesh>
        {/* Left Coarse Knob */}
        <mesh material={darkCharcoalTrim} position={[-0.46, 0, 0]} rotation={[0, 0, Math.PI / 2]}>
          <cylinderGeometry args={[0.22, 0.22, 0.12, 24]} />
        </mesh>
        {/* Left Fine Knob */}
        <mesh material={brassDetail} position={[-0.54, 0, 0]} rotation={[0, 0, Math.PI / 2]}>
